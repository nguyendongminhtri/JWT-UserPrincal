...
//    @Query(value = "SELECT s FROM Student AS s WHERE s.Date >= :startDate AND LAST_UPDATED <= :endDate", nativeQuery = true)
//    List<Student> getAllBetweenDates(@Param("startDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDate startDate,
